 ...
 */
class GeoHelper$Test extends FunSuite {


  test("test if position is in the georec") {

    val ret1 = GeoHelper.isInRec(Position(1,2), Position(2,3), Position(3,4))

    assert(ret1 == false)

    val ret2 = GeoHelper.isInRec(Position(2.5,3.5), Position(2,3), Position(3,4))
  ...
  test("test if at least one of position list is in the georec") {
    ...
    val ret1 = GeoHelper.hasPositionInRec(listPos, Position(0,0), Position(3,3))

    assert(ret1 == true)

    val ret2 = GeoHelper.hasPositionInRec(listPos, Position(4,5), Position(6,7))
  ...
  test("test caculate distance between positions") {
    ...
    val dis = GeoHelper.distance(p1, p2)
  ...
  test("test get geohash with dynamic precision") {
    ...
    val geoHashes = GeoHelper.getGeoHashListInRec(p2, p1)
  ...
  test("test geohash list in a georec") {
    ...
    val ret = GeoHelper.getGeoHashListInRec(bottomLeft, topRight)
  ...
  test("test boundary geohash list in a georec") {
    ...
    val ret2 = GeoHelper.getBoundaryGeoHashListInRec(bottomLeft, topRight, ret)
